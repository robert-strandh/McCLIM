(defpackage #:sheet-geometry-demo
  (:use #:clim-lisp #:clim))
(in-package #:sheet-geometry-demo)
    ...
    (with-bounding-rectangle* (px1 py1 px2 py2) region*
      (with-bounding-rectangle* (x1 y1 x2 y2) region
        (with-drawing-options (stream :transformation transformation)
          ...
          (draw-design stream region :ink +light-slate-blue+ :filled t))
        ...
        (with-drawing-options (stream :transformation transformation)
          ...
          (with-output-as-presentation (stream object 'translate-handler)
            (draw-handler-label stream "Translate" x1 y1 :left :top))
          (with-output-as-presentation (stream object 'rotate-handler)
            (draw-handler-label stream "Rotate" x2 0 :right :top))
          (with-output-as-presentation (stream object 'scale-handler)
            (draw-handler-label stream "Scale" x2 y2 :right :bottom))))
  ...
  (with-bounding-rectangle* (x1 y1 x2 y2) pane
    ...
    (with-drawing-options (pane :clipping-region (sheet-region pane))
      (with-drawing-options
        ...
        (let ((app (find-pane-named frame 'app)))
          (draw-design pane (sheet-region (sheet-parent app)) :ink +blue+ :filled nil)
          (present app))))))
